...
### 1、va中==和equals和hashCode的区别

```
1、==基本数据类型比较的是变量的值
2、==、equals引用数据类型比较的是对象地址
3、String覆写了equals，比较的是字符串内容
```

### 2、JVM内存模型

![1569684950054](C:\Users\ozcom\AppData\Roaming\Typora\typora-user-images\1569684950054.png)

jvm调优工具

--jvisualvm

![1569685429901](C:\Users\ozcom\AppData\Roaming\Typora\typora-user-images\1569685429901.png)

### 3、volatile与JMM内存模型

![1569685068049](C:\Users\ozcom\AppData\Roaming\Typora\typora-user-images\1569685068049.png)

![1569685156405](C:\Users\ozcom\AppData\Roaming\Typora\typora-user-images\1569685156405.png)

![1569685220458](C:\Users\ozcom\AppData\Roaming\Typora\typora-user-images\1569685220458.png)

![1569685253933](C:\Users\ozcom\AppData\Roaming\Typora\typora-user-images\1569685253933.png)

![1569684672260](C:\Users\ozcom\AppData\Roaming\Typora\typora-user-images\1569684672260.png)

### 4、Reference

WeakReference：对象不存在Strong Reference的情况下，只要被gc线程扫描到，无论Stack是否足够，都会立即回收

```java
App app = new App()
WeakReference<App> appWeakReference = new WeakReference<>(app); 弱引用
app = null; //转化为弱引用
System.gc();
```
...
```java
App pApp = new App("ReferenceQueue");

ReferenceQueue<App> rq = new ReferenceQueue<App>();

PhantomReference<App> phantomReference = new PhantomReference<>(pApp, rq);
...
Strong Reference:  只要对象被强引用，minor gc 、major gc、full gc均不会回收，就算是Stack Memory不足、程序抛出OutOfMemoryError异常终止，也不会回收。

```java
Object o=new Object();   //  强引用
o = null;  //弱化引用，使得gc可以回收对象内存
```
...
![ThreadLocal side Map](http://www.jasongj.com/img/java/threadlocal/VarMap.png)
...
![ThreadLocal side Map](http://www.jasongj.com/img/java/threadlocal/ThreadMap.png)
...
非静态内部类：非静态内部类对自己所在的外部类对象***有***一个隐匿的强引用

静态内部类：静态内部类对自己所在的外部类对象***没有***一个隐匿的强引用